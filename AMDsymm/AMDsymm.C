...
void AMDsymm<BasicTurbulenceModel>::calcFilter()
	{
...
template<class BasicTurbulenceModel>
tmp<volScalarField> AMDsymm<BasicTurbulenceModel>::k
(
...
template<class BasicTurbulenceModel>
void AMDsymm<BasicTurbulenceModel>::correctNut()
{
    volTensorField gradU = fvc::grad(this->U_);

    volTensorField numerator1 ( filter_ & symm(gradU) ); 
    
    volTensorField numerator2 ( (filter_ & symm(gradU)) & (filter_ & symm(gradU)) ); 
    
    volScalarField numerator3 ( (filter_ & symm(gradU)) & (filter_ & symm(gradU)) && symm(gradU) ); 

    volScalarField numerator  ( max ( - numerator3,
		    	  dimensionedScalar("numerator", dimensionSet(0, 2, -3, 0, 0, 0, 0), 1e-25) ) );

    volScalarField denominator ( symm(gradU) && symm(gradU) );
...
template<class BasicTurbulenceModel>
AMDsymm<BasicTurbulenceModel>::AMDsymm
(
...
template<class BasicTurbulenceModel>
bool AMDsymm<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
void AMDsymm<BasicTurbulenceModel>::correct()
{
    if (!this->turbulence_)
    {
        return;
